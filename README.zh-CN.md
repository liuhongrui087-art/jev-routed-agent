...
<img src="docs/images/demo-ui.png" alt="对话界面：顶部是检索知识库区块，中间是流式回答，底部是耗时徽章" width="520">

上面这张截图是一道概念题的完整过程：顶部是「📚 检索知识库」区块（**2 个片段，全文可见**），
中间是流式生成的回答，底部是耗时徽章（`31.3s`，橙色）和模式徽章（`Agent`，蓝色）。
...
先看一条真实的服务端日志 —— 这是「Jev 判定」与「预检索」两步都生效的样子：

![服务端日志：Jev 路由到 knowledge，置信度 1.0，预检索命中 2 段](docs/images/demo-router-log.png)

图中三行分别是：路由来源（Jev）、路由结果（`knowledge`，置信度 `1.0`）、
预检索结果（2 段，623 字）。
